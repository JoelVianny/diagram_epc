...
export function circle(svg, canvasData, circleData) {
	const templ = `
		...
		<circle data-key="main" r="24" fill="#D20062" stroke="#fff" stroke-width="1" />
	...
	const shape = shapeCreate(svg, canvasData, circleData, templ,
		{
			right: { dir: 'right', position: { x: 24, y: 0 } },
			left: { dir: 'left', position: { x: -24, y: 0 } },
			bottom: { dir: 'bottom', position: { x: 0, y: 24 } },
			top: { dir: 'top', position: { x: 0, y: -24 } }
		},
		// onTextChange
		txtEl => {
			const newRadius = textElRadius(txtEl, 24, 12);
	...
	function resize() {
		...
		radiusSet(shape.el, 'outer', circleData.r + 12);
		radiusSet(shape.el, 'main', circleData.r);
		shape.draw();
	}

	if (!!circleData.r && circleData.r !== 24) { resize(); } else { shape.draw(); }
