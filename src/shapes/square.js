 ...
 */
export function square(svg, canvasData, rectData) {
	rectData.w = rectData.w ?? 144;
	...
	<rect data-key="main" width="${rectData.w}" height="${rectData.h}" x="${-rectData.w / 2}" y="${-rectData.h / 2}" fill="#5E1675" stroke="#fff" stroke-width="1" />
	...
	const shape = shapeCreate(svg, canvasData, rectData, templ,
		{
			right: { dir: 'right', position: { x: rectData.w / 2, y: 0 } },
		...
		txtEl => {
			...
			const newWidth = ceil(144, 48, textBox.width + (rectData.t ? 6 : 0)); // 6 px right padding for text shape
